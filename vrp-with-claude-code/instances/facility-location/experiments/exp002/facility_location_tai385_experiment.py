#!/usr/bin/env python3
"""
Facility Location Problem experiment with tai385 data using MIP solver
...
def main():
    ...
    print("実験対象: tai385 (384ヶ所の候補地から30ヶ所の施設を選択)")
    ...
    vrp_file = "../../../../instances/tai385/data/tai385.vrp"
    print(f"VRPファイル: {vrp_file}")
    
    # データ抽出（depotを除外して384ヶ所を対象にする）
    ...
    # 設定パラメータ（tai385は大きいので施設数を多く）
    num_facilities = 30    # 設置する施設数
    time_limit = 1200      # 20分の時間制限
    ...
    print(f"施設設置場所: {facility_locations[:10]}{'...' if len(facility_locations) > 10 else ''}")
    ...
    if facility_locations and assignments:
        try:
            ...
            main_viz_file = visualize_facility_location(
                ...
                save_path="facility_location_tai385_solution.png"
            )
            ...
            analysis_viz_file = create_facility_analysis_plot(
                ...
                assignments,
                solution_cost,
                save_path="facility_analysis_tai385.png"
            )
    ...
    with mlflow.start_run(run_name=f"facility_location_tai385_{num_facilities}fac"):
        # パラメータを記録
        mlflow.log_param("problem_type", "Facility_Location")
        mlflow.log_param("solver_type", "MIP")
        mlflow.log_param("instance_name", "tai385")
        ...
        result_file = "facility_location_tai385_result.json"
        ...
        # 問題データサマリーを記録
        problem_summary_file = "problem_summary_tai385.txt"
    ...
    print(f"総実験時間: {total_experiment_time:.2f}秒")
    print(f"施設設置場所（先頭10箇所）: {facility_locations[:10]}")
    ...
    if assignments:
        ...
        for i, facility in enumerate(sorted(facility_locations[:10])):
